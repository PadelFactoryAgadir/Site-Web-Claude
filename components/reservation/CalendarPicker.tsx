...
import { isDayAvailableFromApi, isPeakDay, toIsoDate, type ApiAvailability } from '@/lib/availability';
...
export default function CalendarPicker({ selected, onSelect, accent, apiData }: CalendarPickerProps) {
  ...
  for (let i = 0; i < firstWeekday; i++) cells.push({ date: null, isPadding: true });
  ...
  const goPrev = () => { const next = new Date(viewMonth); next.setMonth(next.getMonth() - 1); setViewMonth(next); };
  const goNext = () => { const next = new Date(viewMonth); next.setMonth(next.getMonth() + 1); setViewMonth(next); };
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-7 gap-1 mb-1.5 text-center">
        {weekdays.map((d, i) => (
          <div key={i} className="text-[9px] uppercase tracking-widest text-white/40 font-bold py-1">{d}</div>
        ))}
      </div>

      <div className="grid grid-cols-7 gap-1">
        {cells.map((cell, idx) => {
          if (cell.isPadding || !cell.date) return <div key={idx} aria-hidden="true" />;
          const date = cell.date;
          const isPast = date.getTime() < today.getTime();
          const available = isDayAvailableFromApi(apiData, date);
          ...
          if (isPast) {
            ...
            stateClasses = `text-white hover:${accentBorder} hover:bg-white/5 cursor-pointer ${peak ? 'ring-1 ring-brand-lime/40' : ''}`;
          }
          ...
          return (
            ...
            >
              {date.getDate()}
              {isToday && !isSelected && <span className={`absolute bottom-0.5 w-1 h-1 rounded-full ${accentBg}`} />}
            </button>
          );
        })}
      </div>
